...
async function computeSeasonStats() {
  ...
  seasonStatsPromise = (async () => {
    ...
    for (let i = 0; i < matchdays.length; i++) {
      ...
      const winners = tips.filter(t => t.dayWin === true || Number(t.dayWin || 0) > 0);
      ...
      tips.forEach(t => {
        ...
        p.highestDayPoints = p.highestDayPoints === null ? tipPoints : Math.max(p.highestDayPoints, tipPoints);
        p.lowestDayPoints = p.lowestDayPoints === null ? tipPoints : Math.min(p.lowestDayPoints, tipPoints);

        if (dayWinVal > 0) {
          p.dayWinnerCount += 1;
          p._currentDayWinStreak += 1;
          p.dayWinStreakBest = Math.max(p.dayWinStreakBest, p._currentDayWinStreak);
        } else {
      ...
      const cumulativeRows = rankRows(
        Object.entries(cumulative).map(([player, points]) => ({ player, points })),
        "points",
        playerNameSort
      );
      cumulativeRows.forEach(r => {
        ...
        p.totalRank += r.rank;
        p.rankCount += 1;
        p.bestRank = p.bestRank === null ? r.rank : Math.min(p.bestRank, r.rank);
        p.worstRank = p.worstRank === null ? r.rank : Math.max(p.worstRank, r.rank);
...
function renderSortableTable({ mountId, tableId, columns, rows, defaultSortKey, defaultSortDir = "desc" }) {
  if (!rows.length) {
  ...
  document.querySelectorAll(`button.sortBtn[data-table="${tableId}"]`).forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      renderSortableTable({ mountId, tableId, columns, rows, defaultSortKey, defaultSortDir });
    });
  });
}
...
async function renderMatchdayStatsTab() {
  ...
  renderSortableTable({
    ...
    columns: [
      ...
      { key: "pointsPerGame", label: "Punkte/Spiel", render: (r) => fmt2(r.pointsPerGame) },
      { key: "playersCount", label: "Spieler" },
      { key: "pointsPerPlayer", label: "Punkte/Spieler", render: (r) => fmt2(r.pointsPerPlayer) }
    ],
    rows: matchdayStats
  });
}
...
async function renderTopsFlopsTab() {
  ...
  const byMetric = (arr, metric, isMin = false) => {
    ...
    return { value: best, rows: arr.filter(x => Number(x[metric] || 0) === best) };
  };
  ...
  $("#mdContent").innerHTML = `
    <div class="kpis gameStatsKpis">
      ${renderKpiPlayers("Tippspielsieger", `${Math.round(winner.value)} Punkte`, winner.rows.map(r => r.player))}
      ${renderKpiPlayers("Meiste Tagessiege", `${mostDayWins.value}`, mostDayWins.rows.map(r => r.player))}
      ${renderKpiPlayers("Bester Punkteschnitt", `${fmt2(bestAvg.value)} P/Spiel`, bestAvg.rows.map(r => r.player))}
      ${renderKpiPlayers("Höchster Punktespieltag", `${Math.round(bestHigh.value)} Punkte`, bestHigh.rows.map(r => r.player))}
...
`).join("");
  ...
  const bonusTab = hasBonusTab() ? `<div class="tab" data-i="bonus" data-type="bonus">Bonus</div>` : "";
  ...
  $("#mdTabs").innerHTML = matchdayTabs + bonusTab + playerStatsTab + matchdayStatsTab + topsFlopsTab;
